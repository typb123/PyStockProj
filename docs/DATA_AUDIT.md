...
- `Volume` and volume moving averages remain in the contract; their liquidity
  and split-unit interpretation remains a separate research choice.
- Cumulative OBV was removed because its value depended on the first row of the
  ...
  assign different values to the same ticker/date. Its temporary bounded
  replacement was subsequently ablated and removed, so the current contract has
  no signed-volume feature.

Completed audit check:

- The corrected 10d and 20d walk-forward results must be rerun under the
  37-feature contract before they can be canonical again. See the
...
walk-forward evaluation is rerun with the 37-feature contract.
