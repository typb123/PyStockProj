...
@pytest.mark.parametrize("random_trials", [1, 5])
def test_prepared_random_top_n_state_matches_legacy_and_is_reused(
    monkeypatch,
    random_trials,
):
    metadata = make_ranked_selection_metadata()
    metadata["relative_momentum"] = [0.01, 0.05, -0.02, 0.04, 0.03, 0.10]
    scores = np.array([0.90, 0.80, 0.70, 0.10, 0.20, 0.30])
    top_n_values = (1, 2)
    legacy_workers = 1 if random_trials == 1 else 2
    legacy = build_top_n_selection_reports(
        metadata,
        scores,
        top_n_values=top_n_values,
        random_seed=7,
        random_trials=random_trials,
        random_trial_workers=legacy_workers,
        prediction_days=10,
    )

    import src.train.evaluation.top_n as top_n

    original_random_reports = top_n._random_top_n_selection_reports
    random_call_count = 0

    def counting_random_reports(*args, **kwargs):
        nonlocal random_call_count
        random_call_count += 1
        return original_random_reports(*args, **kwargs)

    monkeypatch.setattr(
        top_n, "_random_top_n_selection_reports", counting_random_reports
    )
    state = top_n.prepare_random_top_n_evaluation_state(
        metadata,
        top_n_values=top_n_values,
        random_seed=7,
        random_trials=random_trials,
    ...
    assert random_call_count == len(top_n_values)
    cached = top_n.build_top_n_selection_reports(
        metadata,
        scores,
        top_n_values=top_n_values,
        random_seed=7,
        ...
        random_trial_workers=legacy_workers,
        prediction_days=10,
        prepared_evaluation_state=state,
        include_phase_timings=True,
    )
    assert random_call_count == len(top_n_values)
    assert_nested_reports_close(
        {key: value for key, value in cached.items() if key != "phase_timings"},
        legacy,
    )
    assert cached["phase_timings"]["random_baseline_seconds"] == 0.0

    cached["basket_backtest"]["top_1"]["random_baseline"]["random_seed"] = -1
    fresh_cached = top_n.build_top_n_selection_reports(
        metadata,
        scores,
        top_n_values=top_n_values,
        random_seed=7,
    ...
    assert fresh_cached["basket_backtest"]["top_1"]["random_baseline"]["random_seed"] == 7
    changed_scores = np.arange(len(scores), dtype=float)
    changed_cached = top_n.build_top_n_selection_reports(
        metadata,
        changed_scores,
        top_n_values=top_n_values,
        random_seed=7,
    ...
    assert (
        changed_cached["ranked_selection"]["top_1"]["model"]
        != fresh_cached["ranked_selection"]["top_1"]["model"]
    )
    assert_nested_reports_close(
        changed_cached["basket_backtest"]["top_1"]["random_baseline"],
        fresh_cached["basket_backtest"]["top_1"]["random_baseline"],
    )
    assert random_call_count == len(top_n_values)


def test_prepared_random_top_n_state_rejects_input_and_setting_mismatches():
    metadata = make_ranked_selection_metadata()
    scores = np.array([0.90, 0.80, 0.70, 0.10, 0.20, 0.30])
    state = evaluation.prepare_random_top_n_evaluation_state(
        metadata,
        top_n_values=(1,),
        random_seed=7,
        random_trials=2,
    ...
    for changed_metadata, kwargs in (
        (metadata.iloc[::-1].reset_index(drop=True), {}),
        (metadata.assign(excess_forward_return=lambda frame: frame["excess_forward_return"] + 0.01), {}),
        (metadata, {"top_n_values": (2,)}),
        (metadata, {"random_seed": 8}),
        (metadata, {"random_trials": 1}),
        (metadata, {"prediction_days": 20}),
    ):
        changed_scores = scores if len(changed_metadata) == len(scores) else scores
        with pytest.raises(ValueError, match="does not match"):
            ...
            build_top_n_selection_reports(
                changed_metadata, changed_scores, **call_kwargs
            )
